...
**Goal:** A **Star quality** setting (High / Medium / Low) that trades star-drawing time for quality. High is today's renderer, unchanged. Medium and Low cut the star-sprite share of a frame (≈1.1 s of ≈1.45 s for a 1080×1920 HDR frame of the Iris) with a small, measured, quality-gated cost.

**Architecture:**
- **High:** today's renderer, pixel for pixel. At 1080p the stars are computed at the 4K working resolution (Loom's working copy is sized so its largest 16:9 frame is 3840×2160) and downsampled to 1080: each output pixel averages 3×3 samples of the star.
- **Medium:** footprint-matched sampling. Inside a sprite's magnified glow, an output pixel takes only as many samples as its footprint needs.
- **Low:** Medium, plus a static far field (stars that never separate from the zooming backdrop are put back into it), plus the dimmest share of the remaining moving stars put back into the backdrop too.
...
- **High = none of this plan's optimizations.** It is the renderer at `ae1a387`. Its reach skipping, channels drawn together and native bloom are already in, pixel-identical or edge-only.
- **Medium** = footprint-matched sampling (no pre-shrunk sprites: a pre-shrunk sharp core once lost 27% of its peak, see the comment above `Render.mipLevel`).
- **Low** = Medium + static far field + the dimmest `Fly.LOW_DIM_SHARE` (proposed 25%) of the stars still moving, by flux, put back into the backdrop. A star put back is never removed: its light stays in the backdrop, so frame 0 keeps it. It zooms with the nebula instead of flying.
- Far stars in Low lose their twinkle. That is wanted (a static far field).
...
- **Defaults:** the dialog defaults to **Medium**. Code that renders without `opts.starQuality` (tests, old callers) gets **High**, so nothing existing changes silently.
...
7. **Tests that proved less than their labels.** Replaced by:
   - High pixel-identical to the full-count path;
   - per-pixel, per-channel error bounds;
   - occlusion through the real `seenShares` on a scene.
8. **The default quietly changed existing callers.** → Code without `opts.starQuality` gets High. No existing test is relabelled.
...
- `script/lib/Fly.js`:
  ...
  - still-rule constants (`Fly.STILL_PX`, `Fly.STILL_GROWTH`, `Fly.STILL_BRIGHT`, `Fly.STILL_CORE_MODEL`, `Fly.LOW_DIM_SHARE`);
...
**Interfaces — Produces:**
- `Fly.starQuality( level ) -> { footprint: boolean, settle: boolean }`: `"high"`, `"medium"` or `"low"`. Missing or unknown gives High.
- `opts.starQuality`: the level string, read by Tasks 1 and 2.

- [ ] **Step 1: Write the failing tests**

```js
   /* Star quality: High is the renderer as it was; Medium samples glows by their footprint; Low also puts still and dim stars into the backdrop. */
   check( "the three levels", [ Fly.starQuality( "high" ), Fly.starQuality( "medium" ), Fly.starQuality( "low" ) ],
          [ { footprint: false, settle: false }, { footprint: true, settle: false }, { footprint: true, settle: true } ] );
   check( "without a level (old callers, the tests) it is High", [ Fly.starQuality(), Fly.starQuality( "ultra" ) ], [ Fly.starQuality( "high" ), Fly.starQuality( "high" ) ] );
   ( function()
   {
      var spec = { id: "youtube_1080", w: 1920, h: 1080, pingPong: false }, o = { travel: 184, duration: 20, fps: 30, starQuality: "high" };
      check( "the level changes the frames, not only the encode", Fly.frameSignature( o, spec, "k" ) != Fly.frameSignature( Object.assign( {}, o, { starQuality: "low" } ), spec, "k" ), true );
   } )();
```
...
```js
         check( "Star quality: three levels, Medium by default, in the options", [ dlg.starQualityCombo.numberOfItems, dlg.options().starQuality ], [ 3, "medium" ] );
```
...
```js
/*
 * Star quality: how the star sprites are drawn. High is the full renderer --
 * at 1080p the stars are computed at the 4K working resolution and each output
 * pixel averages 3 x 3 samples of them. Medium samples a magnified glow only
 * as densely as its footprint needs (Render.glowSamples). Low also puts the
 * stars that never leave the backdrop, and the dimmest of the moving ones,
 * back into the backdrop (Render.forFlight). Without a level: High.
 */
Fly.STAR_QUALITY = { high: { footprint: false, settle: false }, medium: { footprint: true, settle: false }, low: { footprint: true, settle: true } };
Fly.starQuality = function( level )
{
   return Fly.STAR_QUALITY[level] || Fly.STAR_QUALITY.high;
};
```
...
```js
FlyThrough.STAR_QUALITIES = [ "high", "medium", "low" ];   // the Star quality combo's items, in order
FlyThrough.STAR_QUALITY_DEFAULT = "medium";                 // the dialog's default (code without a level gets High)
```
...
```js
      ...
      [ "High", "Medium", "Low" ].forEach( function( t ) { self.starQualityCombo.addItem( t ); } );
      this.starQualityCombo.currentItem = FlyThrough.STAR_QUALITIES.indexOf( FlyThrough.STAR_QUALITY_DEFAULT );
      this.starQualityCombo.toolTip = "<p><b>High</b>: every star drawn in full (for 1080p, computed at 4K and downsampled). " +
                                      "<b>Medium</b>: a near star's glow sampled only as finely as it needs; faster, the same look. " +
                                      "<b>Low</b>: also leaves the far stars, and the dimmest moving ones, in the backdrop (still, no twinkle); fastest.</p>";
...
- [ ] **Step 5: Commit**: `Fly-Through: a Star quality setting (High, Medium, Low)`.

---

### Task 1: Medium — glow samples matched to the footprint
...
- Produces:
  ...
  - `how.footprint === true` turns the thinning on. Without it every pixel gets the full count, so High is unchanged.
...
```js
   /*
    * Medium: a magnified glow sampled only as densely as its footprint needs.
    * Against the full count (High): per pixel and per channel within 1% of the
    * star's peak, the same light, as steady while it drifts; High never thins.
    */
   ( function()
   {
   ...
   /* High draws exactly what the full count draws; Medium draws within the gate on a whole scene, occlusion included. */
   if ( IN_PIXINSIGHT ) ( function()
   {
      ...
      try
      {
         ...
         var high = pix( Render.frame( sc, 0.9, Object.assign( {}, o, { starQuality: "high" } ), W, H, crop ) );
         check( "High never thins the samples", calls, 0 );
         var med = pix( Render.frame( sc, 0.9, Object.assign( {}, o, { starQuality: "medium" } ), W, H, crop ) );
         check( "Medium does", calls > 0, true );
         Render.glowSamples = function( c ) { return c.nx; };
         var full = pix( Render.frame( sc, 0.9, Object.assign( {}, o, { starQuality: "medium" } ), W, H, crop ) );
         ...
         check( "High is pixel for pixel the full count", same, true );
         check( "Medium within 1% of the brightest pixel everywhere (" + ( 100*worst/top ).toFixed( 2 ) + "%)", worst < 0.01*top, true );
      }
 ...
 * Medium and Low (Fly.starQuality): its footprint in the sprite, cam.f x
...
- [ ] **Step 4: Run all three suites.** Expected: `PASS`, and no existing test changes (they render without a level, which is High).

- [ ] **Step 5: Quality gate on the real Iris (ad hoc)**

Adapt `/tmp/agent-scratch/cmp.js` to render the 1080 vertical HDR frames at t = 0.3, 0.6 and 0.95 at High and at Medium. Report:
...
Gate: per-pixel ≤ 1% of peak, light within 0.5%, steadiness no worse than High's + 0.2%, crops indistinguishable. If any of these fails, revert the task.

- [ ] **Step 6: Commit**: `Fly-Through: Medium star quality samples a glow as densely as its footprint needs`.

---

### Task 2: Low — static far field and the dimmest moving stars into the backdrop
...
```js
   ...
   /*
    * Low: a star that never separates from the zooming backdrop -- never
   ...
   if ( IN_PIXINSIGHT ) ( function()
   {
      ...
      try
      {
         ...
         var flight = { travel: 150, easing: "smoothstep", growth: 0.15, brightening: true, duration: 1, fps: 10, motionBlur: true, starQuality: "low" };
         var times = FlyThrough.flightTimes( flight, hspec );
         var fo = Object.assign( {}, flight, { frameDt: times.frameDt } );
         check( "High and Medium draw every star", [ "high", "medium" ].map( function( l ) { return FlyThrough.flightScene( sc, Object.assign( {}, flight, { starQuality: l } ), hspec ) === sc; } ), [ true, true ] );
...
```js
/*
 * Low star quality's static far field: a star is put back into the backdrop
 ...
 * core's model share at most STILL_CORE_MODEL). Then LOW_DIM_SHARE of the
...
Fly.LOW_DIM_SHARE = 0.25;
```

Replace `Fly.describeStars`:

```js
/* The star counts line; `settled` of the placed stars are in the backdrop (Low star quality), so counted there. */
...
/* `still`, plus the dimmest `share` of the sprites not still (by detected flux): what Low puts into the backdrop. */
...
Render.forFlight = function( sc, opts, outW, outH, crop, flight )
{
   var key = JSON.stringify( [ opts.travel, opts.easing, opts.growth, !!opts.brightening, opts.backdropMotion, !!opts.motionBlur, sc.D,
                               outW, outH, crop.x, crop.y, crop.w, crop.h, flight.frameDt, flight.times, Fly.LOW_DIM_SHARE ] );
   ...
   var settle = Render.dimmestMovers( sc, Render.stillStars( sc, o, outW, outH, crop, flight.times ), Fly.LOW_DIM_SHARE );
...
```js
   // stars Low put back into the backdrop: the sprite rule, where the backdrop carries them (at frame 0 exactly where the sprite was)
 ...
 * and, at Low star quality, with its still and dimmest stars in the backdrop
...
```js
      // at Low star quality the flight decides which stars stay in the backdrop: the counts say so
...
Render the 1080 vertical HDR frames at t = 0, 0.3, 0.6 and 0.95 at Medium and at Low. Report:
...
- [ ] **Step 6: Commit**: `Fly-Through: Low star quality -- a static far field, and the dimmest moving stars in the backdrop`.
...
- [ ] **Step 1: Measure.** Over 60 consecutive frames of the Iris 1080 vertical flight at Medium and at Low, record for each sprite drawn:
